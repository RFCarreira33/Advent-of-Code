...
bool simulateSand(set<Coordenates> &cave, int maxy) {
  Coordenates originalSource = Coordenates(500, 0);
  Coordenates source = Coordenates(500, 0);

  if (cave.find(originalSource) != cave.end()) {
    return false;
  }
  ...
  // small changes from the first part it will always return true
  // unlesss the origial source is filled with sand
  cave.insert(source);
  return true;
}
