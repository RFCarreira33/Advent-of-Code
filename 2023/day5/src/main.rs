...
fn parse_input() -> (Vec<i64>, Vec<Vec<Instructions>>) {
    ...
    let mut index: i64 = -1;
    let seeds = input.next().unwrap().split_whitespace().skip(1).map(|seed| seed.parse::<i64>().unwrap()).collect();
    input.for_each(|line| {
        if line == "" {
            return;
        }
        ...
        let mut ss:i64 = 0;
        let mut len:i64 = 0;
        line.split_whitespace().enumerate().take(3).for_each(|(i, number)| {
...
fn part1(input: Vec<Vec<Instructions>>, mut seeds: Vec<i64>) -> i64 {
    seeds.iter_mut().for_each(|seed| {
        input.iter().for_each(|instruction_vec| {
            instruction_vec.iter().any(|instruction| instruction.compare(seed));
        });
    });
    *seeds.iter().min().unwrap()
}

fn part2(input: Vec<Vec<Instructions>>, mut seeds: Vec<i64>) -> i64 {
    0
}
